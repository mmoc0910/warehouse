...
const mapReceiptToForm = (receipt) => ({
  code: receipt?.code || "",
  warehouse_id: String(receipt?.warehouse_id || receipt?.warehouse?.id || ""),
...
export default function StockReceiptsPage() {
  ...
  const openCreateForm = () => {
    ...
    resetForm(generateDraftCode("PNK-"));
    setOpenForm(true);
  };
  ...
  const openEditForm = async (receiptOrId, options = {}) => {
    setSubmitError("");

    try {
      let receipt = receiptOrId;

      if (!receipt || typeof receipt !== "object" || !receipt.items) {
        const receiptId =
          typeof receiptOrId === "object" ? receiptOrId?.id : receiptOrId;
        const res = await stockReceiptService.detail(receiptId);
        receipt = res.data.data;
      }

      setEditingId(receipt.id);
      setForm(mapReceiptToForm(receipt));
      setReopenDetailId(options.fromDetail ? receipt.id : null);
  ...
  const handleConfirmRemove = async () => {
    ...
    try {
      await stockReceiptService.remove(target.id);
      setRemoveItem(null);
  ...
  const handleExportWord = async () => {
    ...
    try {
      setExporting(true);
      setSubmitError("");
      await exportStockReceiptToWord(detail);
    } catch (err) {
    ...
    <>
      ...
      >
        ...
        <div className="table-wrap">
          ...
          {!loading && !list.length ? (
            <EmptyState message="Chưa có phiếu nhập." />
          ) : null}
        </div>
        ...
        <div className="stock-ui">
          <form className="grid stock-form" onSubmit={submit}>
            ...
            <div className="stock-card">
              <h3 className="stock-card-title">Thông tin phiếu nhập</h3>
              ...
              <div className="stock-grid-3">
                ...
                <div className="stock-field">
                  <label className="stock-label">Ngày nhập</label>
                  <input
                    className="stock-control"
                    type="date"
                    value={form.receipt_date || getTodayString()}
                    onChange={(e) =>
            ...
            <div className="stock-card">
              <h3 className="stock-card-title">Danh sách sản phẩm nhập</h3>
                ...
                <table className="stock-table">
                  <thead>
                    <tr>
                      ...
                      <th style={{ width: 180 }}>Đơn giá nhập</th>
          ...
          <div className="grid">
            ...
            <div className="summary-grid">
              ...
              <div className="summary-card">
                ...
                <strong>{formatDateTimeVN(detail.receipt_date)}</strong>
              </div>
                ...
                <tbody>
                  {detail.items?.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td>{formatNumber(item.unit_cost, 0)}</td>
      ...
      <ConfirmDialog
        ...
        message={`Xóa phiếu nhập ${removeItem?.code}?`}
